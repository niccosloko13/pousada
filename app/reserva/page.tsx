  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-7 md:px-6 md:py-9">
        ...
        <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
          <div className="grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-lg bg-slate-50 px-3 py-2">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Destino</p>
              <p className="mt-0.5 truncate text-sm font-semibold text-slate-900">{destino}</p>
            </div>
            <div className="rounded-lg bg-slate-50 px-3 py-2">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Check-in</p>
              <p className="mt-0.5 text-sm font-semibold text-slate-900">{checkinLabel}</p>
            </div>
            <div className="rounded-lg bg-slate-50 px-3 py-2">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Check-out</p>
              <p className="mt-0.5 text-sm font-semibold text-slate-900">{checkoutLabel}</p>
            </div>
            <div className="rounded-lg bg-slate-50 px-3 py-2">
          ...
          <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-2">
            <p className="text-xs text-slate-700">
              <span className="font-semibold text-slate-900">Regras rápidas:</span> 0-6 não paga · 7-12 meia tarifa · 13+ adulto · casa: mínimo {HOUSE_MIN_GUESTS} pessoas e {HOUSE_MIN_NIGHTS} diárias.
            </p>
          </div>
        ...
        <section className="mt-4 rounded-2xl border border-cyan-100 bg-cyan-50/70 px-4 py-3 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-cyan-800">Resultados encontrados</p>
              <p className="mt-0.5 text-lg font-black text-slate-900 md:text-xl">
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-[1.7fr_0.78fr] lg:items-start">
                ...
                <section key={category} className="space-y-3">
                  <div className="flex flex-wrap items-end justify-between gap-3">
                    <div>
                      ...
                      <h2 className="text-lg font-extrabold text-slate-900 md:text-xl">{categoryLabel(category)}</h2>
                  ...
                  <div className="space-y-3">
                    {categoryRooms.map((item) => {
                      ...
                      return (
                        <article
                          key={room.id}
                          className={`rounded-[1.5rem] border bg-white p-4 shadow-[0_20px_55px_rgba(15,23,42,0.09)] ring-1 ring-slate-900/[0.03] transition hover:-translate-y-0.5 hover:shadow-[0_24px_80px_rgba(8,145,178,0.15)] md:p-5 ${
                            isSelected ? "border-cyan-600 ring-2 ring-cyan-200" : "border-slate-200/90"
                          }`}
                        >
                          <div className="mb-3 grid gap-4 md:grid-cols-[300px_1fr]">
                            <div className="relative h-56 overflow-hidden rounded-2xl ring-1 ring-slate-900/5 md:h-60">
                              <Image src={cover} alt={room.name} fill sizes="(min-width: 768px) 300px, 100vw" className="object-cover" />
                              {isTopPick ? (
                            ...
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                  <h3 className="truncate text-xl font-black tracking-tight text-slate-900">{room.name}</h3>
                                  {isSelected ? (
                                    <span className="rounded-full bg-cyan-100 px-3 py-1 text-xs font-semibold text-cyan-900">Selecionado na busca</span>
                                  ) : null}
                                </div>
                                <p className="mt-1.5 line-clamp-2 text-sm text-slate-600">{room.description}</p>
                                {meta.destaque ? (
                                  <p className="mt-2 inline-flex max-w-full rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-800">
                                    {meta.destaque}
                                  </p>
                                ) : null}
                              </div>
                              ...
                              <div className="w-full shrink-0 text-left sm:w-auto sm:text-right">
                                <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Total da estadia</p>
                                <p className="text-3xl font-black tracking-tight text-cyan-900 md:text-4xl">{pricing ? formatCurrencyBRL(pricing.total) : "—"}</p>
                                <p className="mt-0.5 text-xs font-medium text-slate-600">
                                  {pricing
                                    ? `Total para ${pricing.nights} noite(s) · ${pricing.pricingModel === "por_pessoa" ? "por pessoa" : "por diária"}`
                          ...
                          <div className="mt-3 grid gap-3 rounded-2xl bg-slate-50 p-3.5 md:grid-cols-[1fr_auto] md:items-center">
                            ...
                            <div className="flex flex-col gap-2 md:items-end">
                              {!searchReady ? (
                                ...
                                <CTAButton disabled={!isAvailable} href={`/reserva/checkout?${new URLSearchParams({
                                  ...
                                  quartos,
                                }).toString()}`} className="w-full rounded-xl bg-gradient-to-r from-cyan-700 to-cyan-600 px-5 py-3 text-sm font-extrabold text-white shadow-lg shadow-cyan-900/25 hover:from-cyan-600 hover:to-cyan-500 md:w-auto">
          ...
          <aside className="space-y-3 lg:sticky lg:top-24">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="mb-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-semibold leading-snug text-slate-700">
                {COMPANY_LEGAL_NAME}
                <span className="mt-1 block font-mono text-[10px] font-bold text-slate-800">CNPJ {COMPANY_CNPJ}</span>
              </p>
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-sm font-bold text-slate-900">Reserva direta com a pousada</p>
                  <p className="text-xs text-slate-600">Atendimento humano e confirmação automática após pagamento.</p>
                </div>
              </div>
